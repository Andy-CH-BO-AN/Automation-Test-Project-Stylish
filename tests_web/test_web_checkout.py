...
def test_checkout_with_empty_cart(setup_driver, login):
    ...
    assert alert_text == "尚未選購商品", f"Unexpected alert: {alert_text}"
...
def test_checkout_fail(setup_driver, login, customer_info):
    product_page = ProductPage(setup_driver)
    cart_page = CartPage(setup_driver)
    ...
    with allure.step("Checkout with invalid customer info"):
        product_page.go_to_cart()
        cart_page.fill_checkout_form(customer_info)
        cart_page.click_checkout_button()

    alert_text = cart_page.get_alert()
    logging.info("checkout invalid case: %s", customer_info)
...
@pytest.mark.parametrize(
    ...
    test_data_from_excel.read_data(
        "test_data/Stylish_TestCase.xlsx",
        "Checkout with Valid Value",
    ),
)
@allure.title("checkout success")
def test_checkout_success(setup_driver, login, customer_info):
    product_page = ProductPage(setup_driver)
    cart_page = CartPage(setup_driver)
    index_page = IndexPage(setup_driver)
    ...
    with allure.step("Checkout with valid customer info"):
        product_page.go_to_cart()
        normalized_customer = cart_page.fill_checkout_form(customer_info)
        cart_page.click_checkout_button()
    ...
    with allure.step("Verify thank you page"):
        assert cart_page.checkout_summary_contains(normalized_customer)
